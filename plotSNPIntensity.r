...
ylim.fixed <- 2
sepChar <- ':'
XYfile <- 'out.XY.FORMAT' # Requires POS (position), CHR (chromosome) and XY (intensity as X,Y) columns
GTfile <- 'out.GT.FORMAT' # Requires POS (position), CHR (chromosome) and colID columns
CCfile <- '../RawData/Tanzania_GWAS-2.5M_b37_915.sample' # Requires 'scanID' (IDs) and 'caseorcontrol' ('CASE'/'CONTROL') columns
